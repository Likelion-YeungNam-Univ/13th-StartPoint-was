package startpointwas.domain.general.service;

import org.apache.hc.client5.http.impl.classic.HttpClients;
import org.springframework.http.MediaType;
import org.springframework.http.client.HttpComponentsClientHttpRequestFactory;
import org.springframework.stereotype.Service;
import org.springframework.web.client.RestClient;
import org.springframework.web.util.UriComponentsBuilder;
import startpointwas.domain.general.dto.FootTrafficDto;

import java.net.URI;

@Service
public class FootTrafficService {

    private final RestClient restClient;
    private final String baseUrl = "https://bigdata.sbiz.or.kr";

    public FootTrafficService() {
        var httpClient = HttpClients.custom().build();
        var factory = new HttpComponentsClientHttpRequestFactory(httpClient);
        this.restClient = RestClient.builder()
                .baseUrl(baseUrl)
                .requestFactory(factory)
                .build();
    }

    public FootTrafficDto fetchFootTrafficInfo(String analyNo, String admiCd, String upjongCd) {
        URI uri = UriComponentsBuilder
                .fromUriString(baseUrl + "/gis/simpleAnls/getPopularInfo.json")
                .queryParam("analyNo", analyNo)
                .queryParam("admiCd", admiCd)
                .queryParam("upjongCd", upjongCd)
                .queryParam("mililis", "")
                .queryParam("bizonNumber", "")
                .queryParam("bizonName", "")
                .queryParam("bzznType", "")
                .queryParam("xtLoginId", "")
                .build()
                .toUri();

        return restClient.get()
                .uri(uri)
                .accept(MediaType.APPLICATION_JSON)
                .retrieve()
                .body(FootTrafficDto.class);
    }
}
